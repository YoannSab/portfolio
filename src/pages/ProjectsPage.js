...
const ProjectsPage = () => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        project =>
          ...
          pick(project.description, language).toLowerCase().includes(term) ||
  ...
  }, [searchTerm, selectedProjectTypes, selectedTags, language]);
            ...
            {projectTypes.map(type => (
              <WrapItem key={type}>
                <Tag
                  ...
                  px={4}
                >
                  <TagLabel>{t(`projectTypes.${type}`)}</TagLabel>
                </Tag>
              </WrapItem>
            ))}
          </Wrap>
        ...
        <AnimatePresence>
          {filteredProjects.length > 0 ? (
            ...
            <Flex
              ...
              borderRadius="lg"
            >
              <Heading size="md" mb={4}>{t('projects.noResults')}</Heading>
              <Button onClick={() => {
                setSearchTerm('');
                setSelectedProjectTypes([]);
                setSelectedTags([]);
